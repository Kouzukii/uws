...
void createGroup(const FunctionCallbackInfo<Value> &args) {
	uWS::Group<isServer> *group = hub.createGroup<isServer>(args[0]->IntegerValue(args.GetIsolate()->GetCurrentContext()).FromJust(), args[1]->IntegerValue(args.GetIsolate()->GetCurrentContext()).FromJust());
...
{
	uWS::OpCode opCode = (uWS::OpCode) args[2]->IntegerValue(args.GetIsolate()->GetCurrentContext()).FromJust();
...
void upgrade(const FunctionCallbackInfo<Value> &args) {
	...
	NativeString secKey(args.GetIsolate(), args[2]);
	NativeString extensions(args.GetIsolate(), args[3]);
	NativeString subprotocol(args.GetIsolate(), args[4]);
...
void transfer(const FunctionCallbackInfo<Value> &args) {
	...
	if (args[0]->IsObject()) {
		uv_fileno((handle = getTcpHandle(args[0]->ToObject(args.GetIsolate()->GetCurrentContext()).ToLocalChecked()->GetAlignedPointerFromInternalField(0))), (uv_os_fd_t *) &ticket->fd);
	} else {
		ticket->fd = args[0]->IntegerValue(args.GetIsolate()->GetCurrentContext()).FromJust();
	}
...
void closeSocket(const FunctionCallbackInfo<Value> &args) {
	...
	unwrapSocket<isServer>(args[0].As<External>())->close(args[1]->IntegerValue(args.GetIsolate()->GetCurrentContext()).FromJust(), nativeString.getData(), nativeString.getLength());
}
...
void closeGroup(const FunctionCallbackInfo<Value> &args) {
	NativeString nativeString(args.GetIsolate(), args[2]);
	uWS::Group<isServer> *group = (uWS::Group<isServer> *) args[0].As<External>()->Value();
	group->close(args[1]->IntegerValue(args.GetIsolate()->GetCurrentContext()).FromJust(), nativeString.getData(), nativeString.getLength());
}
...
void broadcast(const FunctionCallbackInfo<Value> &args) {
	uWS::Group<isServer> *group = (uWS::Group<isServer> *) args[0].As<External>()->Value();
	uWS::OpCode opCode = args[2]->BooleanValue(args.GetIsolate()) ? uWS::OpCode::BINARY : uWS::OpCode::TEXT;
	NativeString nativeString(args.GetIsolate(), args[1]);
...
void prepareMessage(const FunctionCallbackInfo<Value> &args) {
	uWS::OpCode opCode = (uWS::OpCode) args[1]->IntegerValue(args.GetIsolate()->GetCurrentContext()).FromJust();
...
void startAutoPing(const FunctionCallbackInfo<Value> &args) {
	uWS::Group<uWS::SERVER> *group = (uWS::Group<uWS::SERVER> *) args[0].As<External>()->Value();
	NativeString nativeString(args.GetIsolate(), args[2]);
	group->startAutoPing(args[1]->IntegerValue(args.GetIsolate()->GetCurrentContext()).FromJust(), std::string(nativeString.getData(), nativeString.getLength()));
}
...
void listen(const FunctionCallbackInfo<Value> &args) {
	uWS::Group<uWS::SERVER> *group = (uWS::Group<uWS::SERVER> *) args[0].As<External>()->Value();
	hub.listen(args[1]->IntegerValue(args.GetIsolate()->GetCurrentContext()).FromJust(), nullptr, 0, group);
}
